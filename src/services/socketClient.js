...
class Wss {
  ...
  getSocket = (url = 'ws://api.weixiaoyi/ws') => {
  ...
  closeAll = (isForce) => {
    if (isForce) {
      ...
      return new Promise((resolve) => {
        ...
        promiseAll.forEach((item = {}) => item.closePromise()
          .then(
            (res) => {
              if (res) {
                i++
      ...
      const promiseAll = []
      return new Promise((resolve, reject) => {
        _.values(this.sockets)
          .forEach((item = {}) => item.listeners.forEach((item) => {
            if (item.unsubscribe) {
